...
class DTCSVM:
  ...
  def __init__(self, data, minPts = 5, kde = False, min_sample = 12, local_std = 2.5, figsize = (8,8), progress = False, density=False, svm_c=1.0, svm_gamma='scale'):
      self.data = data
      self.kde = kde
      self.minPts = minPts
  ...
  def tri_dbscan(self):
    if self.kde == True:
      ...
      for idx, cluster in enumerate(initial_clusters):
        ...
        if len(predict_x) != 0:
          svm = SVC(kernel='rbf', C=self.svm_c, gamma=self.svm_gamma)
          svm.fit(X_train, y_train)
          svm_result = svm.predict(predict_x)
          clusters_df.loc[clusters_df['est_clust'] == 0, 'est_clust'] = svm_result
    ...
    if self.kde == False:
      ...
      if len(predict_x) != 0 and len(X_train) != 0 and np.unique(y_train).shape[0] > 1:
        
        svm = SVC(kernel='rbf', C=self.svm_c, gamma=self.svm_gamma)
        svm.fit(X_train, y_train)
        svm_result = svm.predict(predict_x)
        joined_df.loc[joined_df['est_clust'] == 0, 'est_clust'] = svm_result 
        clusters_df = joined_df[['x','y', 'index','est_clust']]
